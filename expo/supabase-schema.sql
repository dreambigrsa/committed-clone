-- ============================================
-- COMMITTED APP - COMPLETE DATABASE SCHEMA
-- ============================================
...
-- Similar triggers for reel likes
...
-- Advertisements policies (everyone can view active ads)
...
CREATE OR REPLACE FUNCTION get_user_with_relationship(user_id_param UUID)
RETURNS JSON AS $$
DECLARE
  result JSON;
BEGIN
  ...
  FROM users u
  LEFT JOIN relationships r ON r.user_id = u.id AND r.status IN ('pending', 'verified')
...
-- Check for duplicate relationships (cheating detection)
CREATE OR REPLACE FUNCTION check_duplicate_relationships(user_id_param UUID)
RETURNS BOOLEAN AS $$
DECLARE
...
BEGIN
  ...
  FROM relationships
  WHERE user_id = user_id_param AND status IN ('pending', 'verified');
...
-- Insert a super admin user (password should be set via Supabase Auth)
INSERT INTO users (id, full_name, email, phone_number, role, phone_verified, email_verified, id_verified)
VALUES 
  ('00000000-0000-0000-0000-000000000001', 'Super Admin', 'admin@committed.app', '+1234567890', 'super_admin', true, true, true)
...
COMMENT ON TABLE users IS 'User accounts with verification statuses';
COMMENT ON TABLE relationships IS 'Relationship records between users';
COMMENT ON TABLE relationship_requests IS 'Pending relationship confirmation requests';
COMMENT ON TABLE cheating_alerts IS 'Alerts for suspicious activity';
COMMENT ON TABLE notifications IS 'User notifications';
COMMENT ON TABLE posts IS 'Social media posts with images/videos';
COMMENT ON TABLE reels IS 'Short video content';
COMMENT ON TABLE messages IS 'Direct messages between users';
COMMENT ON TABLE advertisements IS 'Admin-managed advertisements';
COMMENT ON TABLE disputes IS 'Relationship disputes requiring resolution';
COMMENT ON TABLE couple_certificates IS 'Digital certificates for verified couples';
